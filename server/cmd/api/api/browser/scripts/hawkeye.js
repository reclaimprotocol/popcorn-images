...
(function () {
  ...
  // Register the class with the Reclaim runtime instead of putting it on
  // window. Anti-bot scanners walking Object.getOwnPropertyNames(window)
  // see no Hawkeye-named globals.
  try {
    if (window.Reclaim && typeof window.Reclaim._registerHawkeyeClass === 'function') {
      window.Reclaim._registerHawkeyeClass(RequestInterceptor);
    } else {
      // Fallback: keep a hidden own property only as a last resort.
      Object.defineProperty(window, 'HawkeyeRequestInterceptor', {
    ...
    window.HawkeyeRequestInterceptor = RequestInterceptor;
  }
